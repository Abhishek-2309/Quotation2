...
firewall_queries = [
    ...
    Only return the year associated with the quotation document itself. 
    If not explicitly stated, leave it blank, return in json with key as: 'Year Quoted' """},
    {"key": "Total Length(LF)", "question": "What is the total linear footage (in LF) of the firewall? Look for summed wall segment lengths or configuration tables. If only panel lengths and quantities are given, calculate as sum of lengths for all the walls."},
    {"key": "Average Height (LF)", "question": "What is the average height (in feet) of the firewall across all sections? Exmaine whether firewall heights are explicitly mentioned and give the average height of all firewalls. If not, search for height values in panel dimension formats (L×H×T where H is second), design notes, or wall specs. Find a single weighted average if multiple heights are given."},
    {"key": "Total SF", "question": "What is the total square footage of the firewall? Look for surface area calculations, pricing tables, or summaries where it is mentioned in 'Sq.ft' or 'Square feet'. If unavailable, compute it as the: Total Length x Average Height. Ensure the result reflects full firewall coverage."},
    ...
    {"key": "Average Unit Price ($/SF)", "question": "What is the average unit cost per square foot of the firewall? Look for $/SF entries or unit pricing in tables. If not directly shown, compute as (Total Price ÷ Total SF). Return a single value obtained after the extraction/calculation."},
    {"key": "Width Range (in)", "question": "What is the thickness or width range (in inches) of the firewall panels? Look for panel specifications in drawings or text, especially in formats like L×H×T where thickness is the third value, or in callouts like ‘6” THK’. If not explicitly clear, give a single best estimate."},
